...
import { shouldShowSubscribeButton } from "@/helper/auth";
...
export default function HeaderAccouncement() {
  ...
  const user = data?.user; // ✅ Use session user directly

  // ✅ FIXED: Only show banner if user should see subscribe button
  const showBanner = shouldShowSubscribeButton(user);
      ...
      {showBanner && (
        <div className="flex w-full items-center justify-center gap-x-3 border-b-1 border-divider bg-background/[0.15] px-6 py-2 backdrop-blur-xl sm:px-3.5">
          <p className="text-small text-foreground">
            <Link className="text-inherit text-sm font-semibold" href="#">
          ...
          <Button
            as={Link}
            className="group relative h-8 overflow-hidden bg-transparent text-small font-semibold"
            color="default"
            endContent={
              <Icon
